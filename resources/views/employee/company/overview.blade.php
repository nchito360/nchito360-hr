                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="card shadow-sm">
                            <div class="card-body">
                                <h6 class="card-title mb-3"><i class="bx bx-map-pin text-warning me-2"></i>All Branches
                                </h6>
                    ...
                    <div class="col-md-6">
                        <div class="card shadow-sm">
                            <div class="card-body">
                                <h6 class="card-title mb-3"><i class="bx bx-sitemap text-primary me-2"></i>All
                                    Departments</h6>
                                <ul class="list-unstyled">
                ...
                <div class="card-body table-responsive">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            // Disable if not allowed to edit or is owner
                            $disabled = (!$canEdit || $isOwner) ? 'disabled' : '';
                            // Disable update button if not allowed
                            $buttonDisabled = (!$canEdit || $isOwner) ? 'disabled' : '';
                            @endphp
                            <tr>
                                <form action="{{ route('employee.team.update', $user->id) }}" method="POST">
                                    ...
                                    <td>
                                        <select name="department" class="form-select form-select-sm" {{ $disabled }}>
                                    ...
                                    <td>
                                        <select name="branch" class="form-select form-select-sm" {{ $disabled }}>
                                            <option value="">Select Branch</option>
                                            @if($user->company && $user->company->branches)
                                            @foreach($user->company->branches as $branch)
                                            <option value="{{ $branch }}"
                                                {{ $user->branch == $branch ? 'selected' : '' }}>
                                                {{ $branch }}
                                            </option>
                                            @endforeach
                                            @endif
                                    ...
                                    <td>
                                        <select name="employment_status" class="form-select form-select-sm"
                                            ...
                                            <option value="probation"
                                                {{ $user->employment_status === 'probation' ? 'selected' : '' }}>
                                                Probation</option>
                                            <option value="permanent"
                                                {{ $user->employment_status === 'permanent' ? 'selected' : '' }}>
                                                Permanent</option>
                                            <option value="contract"
                                                {{ $user->employment_status === 'contract' ? 'selected' : '' }}>Contract
                                            </option>
                                            <option value="intern"
                                                {{ $user->employment_status === 'intern' ? 'selected' : '' }}>Intern
                                            </option>
                                            <option value="terminated"
                                                {{ $user->employment_status === 'terminated' ? 'selected' : '' }}>
                                                Terminated</option>
                                        </select>
                                    </td>
                                    ...
                                    <td>
                                        <select name="privileges" class="form-select form-select-sm" required
                                            {{ $disabled }}>
                                            <option value="">-- Select --</option>
                                            <option value="departmental"
                                                {{ $user->privileges === 'departmental' ? 'selected' : '' }}>
                                                Departmental
                                            </option>
                                            <option value="notifications"
                                                {{ $user->privileges === 'notifications' ? 'selected' : '' }}>
                                                Notifications
                                            </option>
                                            <option value="full" {{ $user->privileges === 'full' ? 'selected' : '' }}>
                                                Full Access
                                            </option>
                                            <option value="none" {{ $user->privileges === 'none' ? 'selected' : '' }}>
                                                No Access
                                            </option>
                                            @if(!$isOwner)
                                            <option value="suspended"
                                                {{ $user->privileges === 'suspended' ? 'selected' : '' }}>
                                                Suspend Account
                                            </option>
                                            <option value="delete"
                                                {{ $user->privileges === 'delete' ? 'selected' : '' }}>
                                                Delete from Company
                                            </option>
                                            @endif
                                        </select>
                            ...
                            <ul>
                                ...
                                <li class="mb-2">
                                    <div>
                                        ...
                                        <small
                                            class="text-muted ms-2">{{ $request->created_at->format('d M Y') }}</small>
                                    </div>
                                    <div>
                                        @if($request->status == 'pending')
                                        <form action="{{ route('employee.company.request.cancel', $request->id) }}"
            ...
            <div class="card h-100">
                ...
                <div class="card-body">
                    <form action="{{ route('employee.company.switch') }}" method="POST">
                        ...
                        <input type="text" name="switch_company_code" id="switch_company_code" class="form-control"
                            required>
        ...
        <div class="col-md-4 mb-4">
            <div class="card h-100">
                ...
                <div class="card-body">
                    ...
                    <!-- Leave Company -->
                    <form action="{{ route('employee.company.leave') }}" method="POST"
